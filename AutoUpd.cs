...
using EmeraldSysPKIBackend.Controllers;
...
{
    public class AutoUpd : IJob
    {
        public async Task Execute(IJobExecutionContext context)
        {
            string accessKeyId = Environment.GetEnvironmentVariable("AWS_S3_ACCESS_KEY_ID");
            string accessKeySecret = Environment.GetEnvironmentVariable("AWS_S3_SECRET_ACCESS_KEY");
            AmazonS3Client s3client = new AmazonS3Client(accessKeyId, accessKeySecret, RegionEndpoint.USWest2);
            ...
            foreach (string caKeyFilePath in caKeyFiles)
            {
                ...
                using (FileStream fs = File.OpenRead(caKeyFilePath))
                {
                    ...
                    using (X509Certificate2 cert = pub.CopyWithPrivateKey(privKey))
                    {
                        ...
                        using (MemoryStream mem = new MemoryStream())
                        {
                            ...
                            try
                            {
                                ...
                                {
                                    BucketName = "esyspki-crl",
                                    Key = caCertFileName + ".crl",
                                    InputStream = mem,
            ...
            await Console.Out.WriteLineAsync("Updated CRLs");
        }
    }
}
